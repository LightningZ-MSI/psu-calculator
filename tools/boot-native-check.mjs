...
export async function checkBootBrowser(root, executable) {
  ...
  try {
    ...
    ` });
    const state = () => evaluate(`(() => { const v=document.querySelector('.psu-boot-canvas'); return {active:document.documentElement.classList.contains('psu-boot-active'), time:Number(v.dataset.frame)/30, width:v.width, muted:true, inert:document.querySelectorAll('[inert]').length, ends:window.__bootEvents.length, modal:!document.querySelector('#disclaimerModal').hidden}; })()`);
    ...
    let st=await state(); check('Native V2 canvas advances during locked startup',st.active && st.time>0 && st.width===1280 && st.muted && st.inert>0);
    ...
    check('Missing image completes safely',await evaluate(`new Promise(resolve=>{const c=document.querySelector('.psu-boot-canvas');c.dataset.mark='missing-image.png';window.startLightningBoot(c,()=>resolve(true));setTimeout(()=>resolve(false),2000);})`));
    ...
    check('Mobile plays without overflow',await evaluate(`Number(document.querySelector('.psu-boot-canvas').dataset.frame)>0 && document.documentElement.scrollWidth<=innerWidth`));
    ...
    for (const [width,height] of [[320,568],[390,844],[430,932],[844,390],[768,1024],[1440,900]]) {
      ...
      check('Proportional native stage '+width+'x'+height,Math.abs(geometry.ratio-(width<height?9/16:16/9))<.002 && Math.abs(geometry.cx-width/2)<1 && Math.abs(geometry.cy-height/2)<1 && geometry.sw<=geometry.iw && geometry.vh<=height+1 && geometry.vw<=width+1);
    ...
    check('No video element or MP4 fetch',await evaluate(`!document.querySelector('video') && !performance.getEntriesByType('resource').some(r=>/\\.mp4/.test(r.name))`));
